...
function TripConsole({ job, error, onOpenTour, loadingSavedTrip = false }) {
  ...
  const snapshotMessage = job?.status === 'partial' ? 'Partial live route — inspect coverage gaps' : sourceFailures ? `Complete trip found — ${sourceFailures} source${sourceFailures === 1 ? '' : 's'} unavailable` : 'All configured collectors completed';
