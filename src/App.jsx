...
const collectorState = (status) => status === 'complete' ? 'healthy' : status === 'failed' ? 'failed' : status || 'standby';
...
function Navigation({ job }) {
  ...
  return <header className="site-header"><a className="brand" href="/" onClick={close} aria-label="TripWeave home"><span className="brand-mark"><Route /></span><span>TRIPWEAVE</span></a><nav className={open ? 'nav-links nav-open' : 'nav-links'} aria-label="Primary navigation"><a href="/#how" onClick={close}>Why TripWeave</a><a href={job?.id ? `/trip/${job.id}#compare` : '/#planner'} onClick={close}>Live comparison</a><a href="/#pipeline" onClick={close}>Data pipeline</a><InlineLink href="/#network" onClick={close}>Source health</InlineLink></nav><button className="menu-button" type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-label={open ? 'Close menu' : 'Open menu'}>{open ? <X /> : <Menu />}</button></header>;
}
...
function OfferFallback({ result }) {
  ...
  return <div className="offer-fallback"><div className="fallback-head"><AlertTriangle /><div><h3>No complete journey can be composed yet.</h3><p>These are the real rows that did return. TripWeave will not invent the missing side of the trip.</p></div></div><div className="raw-offer-columns"><div><span className="detail-label">Transport offers / {transports.length}</span>{transports.slice(0, 8).map((offer) => <a className="raw-offer" href={offer.sourceUrl || '#'} target="_blank" rel="noreferrer" key={offer.id}><ModePills modes={[offer.mode]} /><strong>{offer.operator}</strong><span>{offer.priceText || 'Price unavailable'}</span><small>{offer.departure || 'Time unavailable'} · {offer.source}</small></a>)}</div><div><span className="detail-label">Hotel offers / {hotels.length}</span>{hotels.slice(0, 8).map((hotel) => <a className="raw-offer" href={hotel.sourceUrl || '#'} target="_blank" rel="noreferrer" key={hotel.id}><ModePills modes={['Hotel']} /><strong>{hotel.name}</strong><span>{hotel.priceText || 'Price unavailable'}</span><small>{hotel.location || 'Location unavailable'} · {hotel.source}</small></a>)}</div></div></div>;
}
...
function TripConsole({ job, error, onOpenTour, loadingSavedTrip = false }) {
  ...
  useEffect(() => setSelected(journeys[0]?.id || null), [job?.id, journeys.length]);
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => { try { const response = await fetch(`/api/trips/${job.id}`); const next = await response.json(); if (!response.ok) throw new Error(next.error || 'Could not read trip status.'); setJob(next); } catch (error) { setRequestError(error.message); window.clearInterval(timer); } }, 2400);
    return () => window.clearInterval(timer);
  }, [job?.id, job?.status]);
  const searchTrip = async (query) => {
    setRequestError(null); setTourJourney(null);
    try { const response = await fetch('/api/trips', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(query) }); const next = await response.json(); if (!response.ok && response.status !== 202) throw new Error(next.error || 'The trip could not be started.'); setJob(next); navigate(`/trip/${next.id}`); } catch (error) { setRequestError(error.message); }
  };
  useEffect(() => {
