...
export function buildCollectorUrls(query, origin, destination, { tripAdvisorLocationId = null } = {}) {
  ...
  return {
    ...
    googleFlights: `https://www.google.com/travel/flights?q=Flights%20to%20${encodeURIComponent(destination.iata || destination.name)}%20from%20${encodeURIComponent(origin.iata || origin.name)}%20on%20${query.departDate}%20through%20${query.returnDate}&gl=IN&hl=en`,
