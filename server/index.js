...
import { resolveLocation, findAttractions } from './locations.js';
...
const cachedJobById = (id) => {
  try {
    for (const file of fs.readdirSync(cacheDirectory)) {
      ...
      if (cached?.result?.id === id && Date.now() - cached.at < tripHistoryTtlMs) return cached.result;
    }
...
async function executeTrip(job) {
  try {
    ...
    const urls = buildCollectorUrls(job.query, origin, destination);
    updateJob(job, { stage: 'Triggering live travel collectors', progress: 18 });

    const collectorEntries = Object.entries(COLLECTORS).filter(([key, definition]) => definition.enabled && urls[key]);
    const onCollectorUpdate = (update) => {
      ...
      updateJob(job, { stage: 'Collecting current public offers', progress: Math.min(68, 20 + finished * 9) });
    };
    const results = await Promise.all(collectorEntries.map(([key, definition]) => runCollector(key, definition, urls[key], onCollectorUpdate)));
    ...
    const result = { ...normalized, places: tourPlaces, allPlaces: places, ai };
    const partial = normalized.sources.some((source) => source.status === 'failed') || !normalized.journeys.length;
    updateJob(job, { status: partial ? 'partial' : 'ready', stage: partial ? 'Ready with transparent gaps' : 'Trip ready', progress: 100, result });
    const cachedJob = { at: Date.now(), result: publicJob(job) };
...
app.post('/api/trips', (request, response) => {
  try {
    ...
    if (cached && Date.now() - cached.at < cacheTtlMs) return response.status(200).json({ ...cached.result, cached: true });
